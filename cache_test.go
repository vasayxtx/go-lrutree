...
func BenchmarkCache_Get(b *testing.B) {
	...
	elementCounts := []int{1_000, 10_000, 100_000}
	for _, depth := range depths {
		for _, count := range elementCounts {
			cache, leaves := generateTreeForBench(b, depth, count)
			b.Run(fmt.Sprintf("depth=%d/elements=%d", depth, count), func(b *testing.B) {
				for i := 0; i < b.N; i++ {
...
func BenchmarkCache_Peek(b *testing.B) {
	depths := []int{5, 50} // root is the 1st level
	elementCounts := []int{1_000, 10_000, 100_000}
	for _, depth := range depths {
		for _, count := range elementCounts {
			cache, leaves := generateTreeForBench(b, depth, count)
...
//
// Tree structure:
// This function generates a tree with multiple parallel chains (paths) all starting from
// the common root. Each chain is a straight line with no branches until it reaches maxDepth.
// The function returns only the leaf nodes at the maximum depth level.
//
// Parameters:
// - cache: The cache to populate with nodes
// - rootKey: The key of the root node (must already exist in the cache)
// - maxDepth: The depth of each chain, where 1 means only the immediate children of root
// - targetCount: The approximate number of total nodes to create
//
// ASCII example for maxDepth=3 and 3 chains:
//
//	       root
//	      /  |  \
//	    /    |    \
...
func generateTreeForBench(b *testing.B, maxDepth, targetCount int) (*Cache[string, int], []string) {
	b.Helper()

	cache := NewCache[string, int](targetCount)
	rootKey := "root"
	...
	chainsCount := targetCount / maxDepth
	if targetCount%maxDepth != 0 {
		chainsCount++
	}

	leaves := make([]string, 0, chainsCount)
	nodesCount := 1

	for chainID := 1; chainID <= chainsCount; chainID++ {
		parentKey := rootKey
		for depth := 1; depth <= maxDepth && nodesCount <= targetCount; depth++ {
			nodeKey := fmt.Sprintf("node-%d-%d", chainID, depth)
			nodeVal := chainID*maxDepth + depth
	...
	if cache.Len() != targetCount+1 {
		b.Fatalf("Expected %d nodes in cache, got %d", targetCount+1, cache.Len())
	}
